...
var (
	stdlog = log.New(os.Stdout, "[PARSER]: ", log.LstdFlags)
	errlog = log.New(os.Stderr, "[PARSER:ERROR]: ", log.Ldate|log.Ltime|log.Lshortfile)
)
...
func root(c *router.Control) {
	c.Code(http.StatusOK).Body(fmt.Sprintf("PARSER v%s\n", version.RELEASE))
}
